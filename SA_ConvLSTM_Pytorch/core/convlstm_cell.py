...
class BaseConvLSTMCell(nn.Module):
    ...
    def convlstm_cell(
        ...
        # input_gate = nn.functional.leaky_relu(i_conv + self.W_ci * prev_cell)
        # forget_gate = nn.functional.leaky_relu(f_conv + self.W_cf * prev_cell)

        # Current cell output (state)
        C = forget_gate * prev_cell + input_gate * self.activation(c_conv)

        output_gate = torch.sigmoid(o_conv + self.W_co * C)
        # output_gate = nn.functional.leaky_relu(o_conv + self.W_co * C)
