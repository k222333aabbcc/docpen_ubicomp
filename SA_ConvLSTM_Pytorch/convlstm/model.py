from typing import Optional, Tuple, TypedDict, Union#, NotRequired
import sys
import torch
...
class ConvLSTM(nn.Module):
    ...
    def forward(
        ...
        h = torch.zeros((batch_size, self.out_channels, height, width)).to(X.device)

        # Initialize cell input
        cell = torch.zeros((batch_size, self.out_channels, height, width)).to(X.device)
