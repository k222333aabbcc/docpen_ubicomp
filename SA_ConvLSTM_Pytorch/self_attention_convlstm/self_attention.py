...
class SelfAttention(nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dim: int) -> None:
        ...
        self.query_h = nn.Conv2d(
            input_dim, hidden_dim, 1, padding="same", device="cuda:" + str(DEVICE[0])
        )
        self.key_h = nn.Conv2d(input_dim, hidden_dim, 1, padding="same", device="cuda:" + str(DEVICE[0]))
        self.value_h = nn.Conv2d(input_dim, input_dim, 1, padding="same", device="cuda:" + str(DEVICE[0]))
        self.z = nn.Conv2d(input_dim, input_dim, 1, padding="same", device="cuda:" + str(DEVICE[0]))
