...
class SAMConvLSTM(nn.Module):
    ...
    def forward(
        ...
        self._attention_scores = torch.zeros(
            (batch_size, seq_len, height * width), device="cuda:" + str(DEVICE[0])
        )

        if h is None:
            h = torch.zeros(
                (batch_size, self.out_channels, height, width), device="cuda:" + str(DEVICE[0])
            )
        ...
        output = torch.zeros(
            (batch_size, self.out_channels, seq_len, height, width), device="cuda:" + str(DEVICE[0])
        )
